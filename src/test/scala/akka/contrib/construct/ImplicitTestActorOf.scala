...
object ImplicitTestActorOf extends ImplicitTestActorOf {
  ...
  private def newTestActorRef[A <: Actor](actorOf: ActorOf[A], initialize: => A, name: String = null)
    (implicit sys: ActorSystem): TestActorRef[A] = {
    name match {
      case null => TestActorRef[A](actorOf.configure(Props(initialize)(actorOf.actorClassTag)))
      case _    => TestActorRef[A](actorOf.configure(Props(initialize)(actorOf.actorClassTag)), name)
    }
  }

  class TestActorOfOps[A <: Actor](val actorOf: ActorOf[A]) extends AnyVal {

    def testRef(implicit sys: ActorSystem): TestActorRef[A] =
      newTestActorRef[A](actorOf, actorOf.initialize())

    def testRef(name: String = null)(implicit sys: ActorSystem): TestActorRef[A] =
